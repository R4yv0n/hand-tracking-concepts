...
public struct Gesture
{
    ...
    public float gestureRecognitionTime;
    public Boolean gestureRecognitionTimeOn;
}

public class GestureDetection : MonoBehaviour
{
    [Header("Detection activated")]
    public bool active = false;
    // How much accurate the recognize should be
    [Header("Default gesture values")]
    public float defaultThreshhold = 0.05f;
    public float defaultGestureRecognitionTime = 0f;
    public Boolean defaultGestureRecognitionTimeOn = false;
    ...
    public GestureRecognitionTimer recognitionTimer;
    ...
    public UnityEvent anyRecognize;
    ...
    void Start()
    {
        ...
        StartCoroutine(DelayRoutine(2.5f, SetSkeleton));
    }
    ...
    public void SetSkeleton()
    {
        ...
        initSuccessful = fingerbones.Count > 0;
    }
    ...
    void Update()
    {
        ...
        if (debugMode && Input.GetKeyDown(captureKey))
        {
        ...
        if (initSuccessful && active && hand.HandConfidence != OVRHand.TrackingConfidence.Low)
        {
            ...
            if (hasRecognize)
            {
                if (currentGesture.gestureRecognitionTimeOn && recognitionTimer)
                {
                    recognitionTimer.StartTimer(currentGesture);
                }
                else
                {
                    if (recognitionTimer)
            ...
            else
            {
                ...
                if (done)
                {
                    ...
                    if (previousGesture.gestureRecognitionTimeOn)
                    {
                        recognitionTimer.StopTimer();
                    }
                    ...
                    {
                        previousGesture.onReleased?.Invoke();
                    }
                }
    ...
    Gesture RecordGesture()
    {
        ...
        g.threshold = defaultThreshhold;
        g.gestureRecognitionTimeOn = defaultGestureRecognitionTimeOn;
        g.gestureRecognitionTime = defaultGestureRecognitionTime;
    ...
    void Save()
    {
        gestures.Add(RecordGesture());
    } 
    
    public void ReplaceGesture(String gestureName)
    {
        Gesture gesture = RecordGesture();
        foreach (var g in gestures)
        {
            if (g.name.Equals(gestureName))
            {
                gesture.name = g.name;
                gesture.threshold = g.threshold;
                gesture.onRecognized = g.onRecognized;
                gesture.onReleased = g.onReleased;
